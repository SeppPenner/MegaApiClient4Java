...
public class MegaApiClient implements IMegaApiClient {
    ...
    private static final URI BaseApiUri = new URI("https://g.api.mega.co.nz/cs");
    private static final URI BaseUri = new URI("https://mega.nz");
    ...
    #else
    public void DownloadFile(INode node, string outputFile, CancellationToken 
...
#else
      using(Stream 
        stream = this.Download(node, cancellationToken))
#endif
        {
    ...
    #else
    public void DownloadFile(Uri uri, string outputFile, CancellationToken 
        ? cancellationToken = null)
#endif
    {
      if (uri == null) {
...
#else
        using(Stream 
        stream = this.Download(uri, cancellationToken))
#endif
        {
            this.SaveStream(stream, outputFile);
        }
    ...
    #else
    public Stream Download(INode node, CancellationToken 
        ? cancellationToken = null)
#endif
    {
      if (node == null) {
    ...
    #else
    public Stream Download(Uri uri, CancellationToken 
        ? cancellationToken = null)
#endif
    {
      if (uri == null) {
        ...
        string id;
        byte[] iv, metaMac, key;
        this.GetPartsFromUri(uri, out id
        , out iv, out metaMac
        , out key
        );
    ...
    public IEnumerable<INode> GetNodesFromLink(Uri uri) {
        ...
        string shareId;
        byte[] iv, metaMac, key;
        this.GetPartsFromUri(uri, out shareId
        , out iv, out metaMac
        , out key
        );
    ...
    #else
    public INode UploadFile(string filename, INode parent, CancellationToken 
        ? cancellationToken = null)
#endif
    {
...
#
    ...
    public INode Upload(Stream stream, string name, INode parent, DateTime
...
#endif
    {
      if (stream == null) {
        ...
        #
        endif string completionHandle = string.Empty;
                        ...
                        )
            {
              uri = new Uri(uploadResponse.Url + "/" + chunkStartPosition);
                        ...
                        try {
                            ...
                            long retCode;
                            if (completionHandle.FromBase64().Length != 27 && long.TryParse
                                (completionHandle, out  retCode
                                ...
                                ))
                {
                  apiResult = (ApiResultCode) retCode;
                            ...
                            .Invoke(this, new ApiRequestFailedEventArgs(uri, remainingRetry, requestDelay, apiResult, ex));
                ...
                if (apiResult != ApiResultCode.Ok) {
                    this.ApiRequestFailed ?
                    .Invoke(this, new ApiRequestFailedEventArgs(uri, remainingRetry, requestDelay, apiResult, completionHandle));
    ...
    private TResponse Request
    <TResponse
    >(RequestBase request, object context  = null
    )
            where TResponse
            ...
            -- > 0)
      {
                ...
                ))
        {
          ApiResultCode apiCode = jsonData == null
                        ...
                        : jsonData is long
              ?(ApiResultCode
                )Enum.ToObject(typeof(ApiResultCode), jsonData)
              : (ApiResultCode)((JArray)jsonData)[0].Value<int>
                ();

          if (apiCode != ApiResultCode.Ok) {
                    this.ApiRequestFailed ?
                    .Invoke(this, new ApiRequestFailedEventArgs(uri, this.options.ApiRequestAttempts - remainingRetry, requestDelay, apiCode, dataResult));
                }
    ...
    == typeof(string)) ? data as TResponse : JsonConvert.DeserializeObject<TResponse>(data, settings);
    }
    ...
    private void GetPartsFromUri(Uri uri, out string
    id, out byte[] iv, out byte[] metaMac, out byte[] key
        ...
        )
    {
      Regex uriRegex = new Regex("#(?<type>F?)!(?<id>.+)!(?<key>.+)");
        ...
        if (isFolder) {
            ...
            Crypto.GetPartsFromDecryptedKey(decryptedKey, out iv
            , out metaMac, out key
